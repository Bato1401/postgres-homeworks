...
ALTER TABLE order_details DROP CONSTRAINT fk_order_details_products; -- сняли ограничения в order_details
DELETE FROM products WHERE discontinued = 1; -- удалили все снятые с продажи продукты
DELETE FROM order_details WHERE EXISTS(     -- удалили все id удаленных выше продуктов из order_details
	SELECT product_name
	FROM order_details
	LEFT JOIN products USING(product_id)
	WHERE product_name IS NULL);
ALTER TABLE order_details ADD CONSTRAINT fk_order_details_products -- восстановили связь между таблицами
